...
interface IExternalStorage {
    function storeValue(bytes calldata value) external;
}
...
contract WeatherConsumer is FunctionsClient, ConfirmedOwner {
    ...
    IExternalStorage[] public externalStorages;

    function addExternalStorage(address _externalStorage) external onlyOwner {
        externalStorages.push(IExternalStorage(_externalStorage));
    }

    function removeExternalStorage(uint index) external onlyOwner {
        require(index < externalStorages.length, "Index out of bounds");
        externalStorages[index] = externalStorages[externalStorages.length - 1];
        externalStorages.pop();
    }
    ...
    function fulfillRequest(bytes32 requestId, bytes memory response, bytes memory err) internal override {
        ...
        if (err.length == 0) {
            for (uint i = 0; i < externalStorages.length; i++) {
                externalStorages[i].storeValue(response);
            }
        }
    }
}
